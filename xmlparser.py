...
for daterange in root.iter('date_range'):
    for begin in daterange.iter('begin'):
        beginDate = time.strftime("%Y-%m-%d", time.localtime(int(begin.text)))
        print(beginDate)
    for end in daterange.iter('end'):
        endDate = time.strftime("%Y-%m-%d", time.localtime(int(end.text)))
        print(endDate)
...
reportInsert = "insert into post(title, content, category_id) value(%s, %s, %s, %s)"
